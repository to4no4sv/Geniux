...
class GetAnnotation:
    @asyncFunction
    async def getAnnotation(self, id: int, textFormat: TextFormat = None) -> Union[Annotation, None]:
        annotation = await self._req(
            f"referents/{id}",
            {
        ...
        annotation = annotation.get("referent")
        if not annotation:
            return

        return self._finalizeResponse(annotation, Annotation)