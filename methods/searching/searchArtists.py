...
class SearchArtists:
    @asyncFunction
    async def searchArtists(self, query: str, perPage: int = None, page: int = None, sort: Sort = None) -> Union[List[Artist], Artist, None]:
        searchResults = await self._search(query, "/artist", perPage, page, sort)

        return self._finalizeResponse(searchResults, Artist)