...
from geniux.types import User
from geniux.enums import Sort

class SearchUsers:
    @asyncFunction
    async def searchUsers(self, query: str, perPage: int = None, page: int = None, sort: Sort = None) -> Union[List[User], User, None]:
        searchResults = await self._search(query, "/user", perPage, page, sort)

        return self._finalizeResponse(searchResults, User)