...
class SearchAlbums:
    @asyncFunction
    async def searchAlbums(self, query: str, perPage: int = None, page: int = None, sort: Sort = None) -> Union[List[Album], Album, None]:
        searchResults = await self._search(query, "/album", perPage, page, sort)

        return self._finalizeResponse(searchResults, Album)